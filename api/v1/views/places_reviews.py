#!/usr/bin/python3
""" Creates a view for Review objects - handles all default RESTFUL API actions"""
...
def get_reviews_by_place(city_id):
...
def delete_review(review_id):
    ...
    review = storage.get(Review, review_id)
    if review:
        # Deletes the Review object from the storage and save changes
...
def create_review(place_id):
    ...
    review.save()
    # Return the newly created Review object in JSON format with 201 status code
...
def update_review(review_id):
    ...
    if review:
        ...
        # Get the JSON data from the request
        data = request.get_json()
        ignore_keys = ['id', 'user_id', 'place_id', 'created at', 'updated at']
       ...
       for key, value in data.items():
        ...
        review.save()
        # Return the updated Review objectin JSON format with 200 status code
